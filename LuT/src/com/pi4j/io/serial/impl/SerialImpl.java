...
/**
 * Modified version of original pi4j class. Only difference is the omitted ShutdownHook.
 */
...
public class SerialImpl extends AbstractSerialDataReaderWriter implements Serial {
	protected int fileDescriptor = -1; // SUPPRESS_CHECKSTYLE
	protected final CopyOnWriteArrayList<SerialDataEventListener> listeners; // SUPPRESS_CHECKSTYLE
	protected final ExecutorService executor; // SUPPRESS_CHECKSTYLE
	protected final SerialByteBuffer receiveBuffer; // SUPPRESS_CHECKSTYLE
	protected boolean bufferingDataReceived = true; // SUPPRESS_CHECKSTYLE
	...
	/**
	 * Utility method required due to use of ShutdownHook in custom class.
	 */
	 ...
	 */
	@Override
	public void open(final String device, final int baud, final int dataBits, final int parity, final int stopBits, final int flowControl)
			throws IOException {
		...
		if (available > 0) {
			byte[] initial_data = com.pi4j.jni.Serial.read(fileDescriptor, available); // SUPPRESS_CHECKSTYLE
		...
		SerialInterrupt.addListener(fileDescriptor, new SerialInterruptListener() {
			@Override
			public void onDataReceive(final SerialInterruptEvent event) {
	 ...
	 */
	@Override
	public void open(final String device, final int baud) throws IOException {
	 ...
	 */
	@Override
	public void open(final String device, final Baud baud, final DataBits dataBits, final Parity parity, final StopBits stopBits,
			final FlowControl flowControl) throws IOException {
	 ...
	 */
	@Override
	public void open(final SerialConfig serialConfig) throws IOException {
	 ...
	 */
	@Override
	public void sendBreak(final int duration) throws IllegalStateException, IOException {
		// validate state
		if (isClosed()) {
	 ...
	 */
	@Override
	public void setBreak(final boolean enabled) throws IllegalStateException, IOException {
		// validate state
		if (isClosed()) {
	 ...
	 */
	@Override
	public void setRTS(final boolean enabled) throws IllegalStateException, IOException {
		// validate state
		if (isClosed()) {
	 ...
	 */
	@Override
	public void setDTR(final boolean enabled) throws IllegalStateException, IOException {
		// validate state
		if (isClosed()) {
	 ...
	 */
	@Override
	public byte[] read(final int length) throws IllegalStateException, IOException {
		// validate state
		if (isClosed()) {
			throw new IllegalStateException("Serial connection is not open; cannot 'read()'.");
		}
		...
		int b = receiveBuffer.getInputStream().read(buffer, 0, length);
		if (b < 0) {
			// Dummy code due to FindBugs
			return buffer;
		}
		return buffer;
	}
	 ...
	 */
	@Override
	public void write(final byte[] data, final int offset, final int length) throws IllegalStateException, IOException {
		// validate state
		if (isClosed()) {
	 ...
	 */
	@Override
	public synchronized void addListener(final SerialDataEventListener... listener) {
	 ...
	 */
	@Override
	public synchronized void removeListener(final SerialDataEventListener... listener) {
	 ...
	 */
	@Override
	public void setBufferingDataReceived(final boolean enabled) {
		bufferingDataReceived = enabled;
	}

	private class SerialOutputStream extends OutputStream { // SUPPRESS_CHECKSTYLE

		@Override
		public void write(final byte b[]) throws IOException { // SUPPRESS_CHECKSTYLE
		...
		@Override
		public void write(final byte b[], final int offset, final int length) throws IOException { // SUPPRESS_CHECKSTYLE
	...
	@SuppressWarnings("unused") // SUPPRESS_CHECKSTYLE
